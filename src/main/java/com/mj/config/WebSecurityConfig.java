...
import com.mj.handler.MJAuthenctiationFailureHandler;
import com.mj.handler.MJAuthenticationSuccessHandler;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/oauth/**", "/login/**", "/logout").permitAll().anyRequest()
				...
				.successHandler(authenticationSuccessHandler).permitAll().failureHandler(authenctiationFailureHandler)
				.and().logout().permitAll().logoutUrl("/logout").logoutSuccessUrl("/login").and().csrf().disable();
	}
